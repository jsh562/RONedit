...
//! `ron-core` — it holds only CST + text + a generic cursor value, so the crate
...
//! so `ron-core` stays decoupled from any surface's cursor representation: the
...
/// (`cursor`). Generic over the cursor/metadata type `C` so `ron-core` stays
...
/// in `ron-core` precisely so any surface can own a per-document handle, while the
