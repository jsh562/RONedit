...
//!
//! This module gives `ron-core` the **named** structural-edit vocabulary the
...
//! ADR-0007: they live in `ron-core` (reusable by a future LSP/web surface),
...
//! dependency. It uses only `std` and `ron-core`'s own CST types + `apply_edit`,
//! so the `wasm32-unknown-unknown` build of `ron-core` stays green.
//!
//! # Addressing scheme
//!
