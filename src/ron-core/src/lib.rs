//! `ron-core` — a lossless, error-tolerant concrete-syntax-tree (CST) engine for
//! RON (Rusty Object Notation).
//!
//! `ron-core` parses RON source into a CST that preserves **every byte** of the
...
//!   API; the CST is exposed through `ron-core`'s own [`SyntaxNode`] /
...
//!
//! `ron-core`'s public surface is **0.x shape-stable**: the *capability areas* —
...
//! reachable only through `ron-core`'s own opaque [`SyntaxNode`] / [`SyntaxToken`]
...
//!
//! `ron-core`'s only runtime dependency is `rowan` (pure Rust); the crate carries
//! no filesystem / UI / async-runtime / native dependency, so it builds for
//! `wasm32-unknown-unknown`:
//!
//! ```text
//! rustup target add wasm32-unknown-unknown
//! cargo build -p ron-core --target wasm32-unknown-unknown   # MUST succeed
//! ```
//!
...
//! assert_eq!(ron_core::print(&doc), src); // byte-for-byte round-trip
//! ```
