...
//! public `ron_core` API to parse, navigate, read diagnostics, print, and edit.
...
use ron_core::ast::{Document, Value};
use ron_core::{
...
/// consumer touches. Naming them all in one `ron_core`-only type is a
...
fn parse_navigate_diagnostics_print_round_trip() {
    ...
    // through the `ron_core`-only `PublicApiWitness` alias (INV-7).
