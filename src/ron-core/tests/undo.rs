...
//! [`ron_core::UndoStack`]. The editor-level half (driving undo/redo through
...
fn exceeding_count_cap_drops_oldest() {
    let cap = UndoCap::new(4, ron_core::undo::DEFAULT_UNDO_BYTE_CAP);
...
fn misconfigured_cap_falls_back_to_default_never_unbounded() {
    ...
    assert_eq!(
        stack.cap().max_count,
        ron_core::undo::DEFAULT_UNDO_COUNT_CAP
    );
    assert_eq!(stack.cap().max_bytes, ron_core::undo::DEFAULT_UNDO_BYTE_CAP);
}
...
proptest! {
    ...
    fn prop_ring_stays_within_count_cap_and_drops_oldest(
        ...
        let cap = UndoCap::new(cap_count, ron_core::undo::DEFAULT_UNDO_BYTE_CAP);
        let mut stack: UndoStack<usize> = UndoStack::with_config(cap, WINDOW);
        for i in 0..edits {
