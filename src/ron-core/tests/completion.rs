...
//!
//! Exercises the **public** `ron_core::completion` surface only (no internal
...
use ron_core::{completion_context, completions, parse, CompletionKind, PositionKind};

/// The completion context at `offset` bytes into freshly-parsed `src`.
fn ctx(src: &str, offset: usize) -> ron_core::CompletionContext {
