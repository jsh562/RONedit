//! cargo-fuzz no-panic + lossless-round-trip target for `ron-core` (SC-003).
//!
...
//! proptest in `src/ron-core/tests/roundtrip.rs` (the "T028 — stable-toolchain
...
fn token_concat(doc: &ron_core::CstDocument) -> String {
    doc.root()
        .descendant_tokens()
...
fuzz_target!(|data: &[u8]| {
    match ron_core::parse_bytes(data) {
        Ok(doc) => {
            ...
            assert_eq!(ron_core::print(&doc), s, "print must round-trip");
