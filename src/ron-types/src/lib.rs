//! `ron-types` — RONin's normalized RON type model (native-gated, E004).
//!
...
//!
//! - **Native-gated (ADR-0002/0004).** `ron-types` MUST NOT be a dependency of
//!   the WASM-clean `ron-core`. The only artifact that crosses to `wasm32` is
...
//! # Deferred seams
//!
//! `ron-types` deliberately stops at *acquiring, normalizing, and serializing*
...
//!   `ron-core`. `ron-types` only produces the schema; it never executes
