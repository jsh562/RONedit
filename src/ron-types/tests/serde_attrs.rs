...
use ron_types::model::{Discriminator, NodeKind, VariantShape};
use ron_types::source::TypeSource;
...
fn model() -> ron_types::TypeModel {
    SynSource::from_path(fixture()).acquire().model
}

fn object_fields(node: &ron_types::TypeNode) -> &[ron_types::model::Field] {
    match &node.kind {
