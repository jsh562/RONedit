...
//!
//! `ron-types` is **native-only** by design: it depends on `syn` / `walkdir`,
...
//! WASM-clean `ron-core` consumer added in E006, which depends only on
//! `serde_json` (no `ron-types`, `syn`, `walkdir`, or `schemars`).

use ron_types::extension::RonTypeExtension;
use ron_types::model::{
    Discriminator, Field, NodeKind, Primitive, TypeModel, TypeNode, TypeRef, Variant, VariantShape,
};
use ron_types::serialize::{from_json, from_json_str, to_json, to_json_string};
use ron_types::AcquisitionDiagnostic;
...
fn populated_model() -> TypeModel {
    ...
    model.diagnostics.push(AcquisitionDiagnostic::new(
        ron_types::diagnostics::DiagnosticCategory::UnresolvedType,
        "Foreign",
