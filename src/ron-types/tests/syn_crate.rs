...
use ron_types::model::{NodeKind, Primitive, TypeRef, VariantShape};
use ron_types::source::{SourcePrecedence, TypeSource};
use ron_types::SynSource;
...
fn unresolved_types_diagnosed_but_no_fatal_errors() {
    ...
    let unresolved = acq
        .diagnostics
        .iter()
        .filter(|d| d.category == ron_types::diagnostics::DiagnosticCategory::UnresolvedType)
        .count();
    assert!(
        unresolved >= 2,
