//! Native-gating invariant: `ron-core` must NOT depend on `ron-types` {TR-013}.
//!
//! `ron-types` is native-only (it pulls in `syn` / `walkdir` / `schemars`, none
//! of which build for `wasm32`). The WASM-clean `ron-core` (project-instructions
//! §II, ADR-0002) MUST therefore never depend on `ron-types` or any of those
//! native crates — otherwise `ron-core`'s `wasm32` build would break.
//!
//! This test is **hermetic and fast**: it reads `ron-core`'s `Cargo.toml` from
...
//! `wasm32-unknown-unknown` build of `ron-core` (T036), which fails if any native
//! dependency leaks in.

use std::path::PathBuf;

/// Crates that must never appear in `ron-core`'s dependency tables.
const FORBIDDEN: &[&str] = &["ron-types", "syn", "walkdir", "schemars"];
...
fn ron_core_manifest_path() -> PathBuf {
    // CARGO_MANIFEST_DIR = .../src/ron-types ; ron-core is its sibling.
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    manifest_dir
        .parent()
        .expect("ron-types crate dir must have a parent (the workspace src/ dir)")
        .join("ron-core")
        .join("Cargo.toml")
}
...
#[test]
fn ron_core_does_not_depend_on_ron_types_or_native_crates() {
    let manifest_path = ron_core_manifest_path();
    let manifest = std::fs::read_to_string(&manifest_path).unwrap_or_else(|e| {
        panic!(
            "could not read ron-core manifest at {}: {e}",
    ...
    for forbidden in FORBIDDEN {
        assert!(
            ...
            "ron-core must stay WASM-clean (TR-013): it must NOT declare a \
...
/// Sanity check that the parser actually found `ron-core`'s real dependency
...
#[test]
fn parser_sees_ron_core_real_dependencies() {
    let manifest_path = ron_core_manifest_path();
    let manifest = std::fs::read_to_string(&manifest_path).expect("ron-core manifest readable");
    let deps = declared_dependency_names(&manifest);
    assert!(
        deps.iter().any(|d| d == "rowan"),
        "expected to parse ron-core's `rowan` dependency; parsed: {deps:?}"
    );
}
